...
public class RowCache {
    ...
    public RowCache(Context cxt, String tableName, PipeLine pipeLine) {
        ...
        this.mergeInterval = cxt.conf.mergeInterval;
        this.batchSize = cxt.conf.batchSize;

        this.rows = new LinkedBlockingQueue<Row>(cxt.conf.batchSize * 2);
        this.commitCallback = new LinkedBlockingQueue<CommitCallback>();

        this.localMerger = new LinkedList<RowBatchMerger>();
        for (int i = 0; i < cxt.conf.loaderTaskQueueSize; i++) {
            localMerger.add(new RowBatchMerger());
        }

        this.mergeResult = new LinkedBlockingQueue<Future<LoadTask>>(cxt.conf.loaderTaskQueueSize);
    ...
    public boolean shouldMerge() {
        if (System.currentTimeMillis() - lastMergeTime < mergeInterval && rows.size() < batchSize) {
            return false;
        }
        return true;
    }
