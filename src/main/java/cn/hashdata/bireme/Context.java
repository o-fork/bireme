...
public class Context {
     ...
     */
    public Context(BiremeConfig conf) throws BiremeException {
        this.conf = conf;

        this.tableMap = conf.tableMap;
        this.tablesInfo = new HashMap<String, Table>();

        this.loaderConnections = new LinkedBlockingQueue<Connection>(conf.loaderConnSize);
        ...
        this.server = new StateServer(this, conf.stateServerAddr, conf.stateServerPort);
    ...
    private void createThreadPool() {
        pipeLinePool =
                Executors.newFixedThreadPool(conf.pipelinePoolSize, new BiremeThreadFactory("PipeLine"));
        transformerPool = Executors.newFixedThreadPool(
                conf.transformPoolSize, new BiremeThreadFactory("Transformer"));
        mergerPool =
                Executors.newFixedThreadPool(conf.mergePoolSize, new BiremeThreadFactory("Merger"));
        loaderPool =
                Executors.newFixedThreadPool(conf.loaderConnSize, new BiremeThreadFactory("Loader"));
    }
