...
public class Bireme implements Daemon {
     ...
     * 主要是为了获取配置文件路径
     */
    private void parseCommandLine(String[] args) throws DaemonInitException, ConfigurationException, BiremeException {
        Option help = new Option("help", "print this message");

        Option configFile = Option.builder("config_file")
                .hasArg()
                .argName("file")
        ...
        cxt = new Context(new BiremeConfig(config));
    }
    ...
    protected void initLoaderConnections() throws BiremeException {
        ...
        try {
            for (int i = 0, number = cxt.conf.loaderConnSize; i < number; i++) {
        ...
        logger.info("Finishing establishing {} connections for loaders.", cxt.conf.loaderConnSize);
    }
    ...
    protected void createPipeLine() throws BiremeException {
        for (SourceConfig conf : cxt.conf.sourceConfig.values()) {
            switch (conf.type) {
                case MAXWELL:
                    KafkaConsumer<String, String> consumer = KafkaPipeLine.createConsumer(conf.server, conf.groupID);
        ...
        switch (cxt.conf.reporter) {
            case "console":
                consoleReporter = ConsoleReporter.forRegistry(cxt.register)
                        ...
                        .build();
                consoleReporter.start(cxt.conf.reportInterval, TimeUnit.SECONDS);
                break;
            case "jmx":
